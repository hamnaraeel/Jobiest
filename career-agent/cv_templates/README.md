...
`escape_latex()`), then substitutes exactly three placeholders into the
template:

- `{{NAME}}` -- the candidate's name
- `{{CONTACT_LINE}}` -- email / phone / links, joined
- `{{BODY}}` -- every included section's rendered LaTeX, concatenated in
  the order the CV plan decided (`CVContent.section_order`)
...
1. Add a `.tex` file under a subdirectory here (e.g. `ats/research.tex`)
   with the same three placeholders.
